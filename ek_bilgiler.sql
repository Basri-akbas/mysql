...
select concat('Adiniz Soyadiniz:',ad ,' ',soyad ) as ad_soyad from customer;
...
select ad, left(ad,1), soyad ,left(soyad,1) from customer; -- left soldan baslar kac harf istersek o kadar alir. 
...
select  ad,concat(left(ad,1),right(left(ad,3),1),right(left(ad,5),1)) as harfler from customer where length(ad)>=5;
